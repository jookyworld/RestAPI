...
public class BaseInitData {
    ...
    public void work1() {
        ...
        if (AppConfig.isNotProd()) user1.modifyApiKey(user1.getUsername());
        Member user2 = memberService.join("user2", "1234", "유저2");
        if (AppConfig.isNotProd()) user1.modifyApiKey(user2.getUsername());
        Member user3 = memberService.join("user3", "1234", "유저3");
        if (AppConfig.isNotProd()) user1.modifyApiKey(user3.getUsername());
        Member user4 = memberService.join("user4", "1234", "유저4");
        if (AppConfig.isNotProd()) user1.modifyApiKey(user4.getUsername());
        Member user5 = memberService.join("user5", "1234", "유저5");
        if (AppConfig.isNotProd()) user1.modifyApiKey(user5.getUsername());
    }
